// src/app/quality/QualityAnimations.tsx
'use client'
...
export default function QualityAnimations({
	...
	useLayoutEffect(() => {
		if (!qualityData) return // Не запускать анимации пока нет данных
		...
		const ctx = gsap.context(() => {
			// === SECOND HERO — КАК В aboutUs ===
			{
				...
				if (hero) {
					...
					const rightWrap = hero.querySelector<HTMLElement>(
						`.${styles.rightImg}`
					)
		...
		return () => ctx.revert()
	}, [qualityData]) // Зависимость от qualityData
